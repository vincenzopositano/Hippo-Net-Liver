...
#tf.config.set_visible_devices([], 'GPU')    # run on CPU
    ...
    for i in range(dataSize):
        ...
        for j in range(inc):
            ...
            if (p ==2):
                augImg=tf.keras.preprocessing.image.random_zoom(augImg,[1.0,1.6],channel_axis=0,fill_mode='nearest')
            if (p ==3):
                    augImg=tf.keras.preprocessing.image.random_shift(augImg,0.1,0.1,channel_axis=0,fill_mode='nearest')
            if (p ==4):
                    rfloat = 1+rng.random()
                    augImg=augImg*rfloat
            if (p ==5):
...
model.add(ConvLSTM2D(filters = bSize, kernel_size = (kSize,kSize), padding='same',return_sequences = True, data_format = "channels_last",kernel_initializer='he_normal',input_shape = (seq_len, img_height, img_width,1)))
...
history = model.fit(x = X_train, y= y_train, epochs=epochs,batch_size = batchSize , class_weight=classW,steps_per_epoch=40,shuffle=True, validation_data=(X_val,y_val),callbacks=callbacks)
...
model.save('/home/positano/Desktop/DEEP_LEARNING/model_current_LSTM_88')
...
print('Accuracy val set ',accuracy_score(y_val_label,y_pred_label))
...
print('Accuracy test set= ',accuracy_score(y_test_label,y_pred_label))

confusionMatrix=confusion_matrix(y_test_label,y_pred_label,labels=classes)
disp = ConfusionMatrixDisplay(confusion_matrix=confusionMatrix,display_labels=classes)
disp.plot()

confusionMatrix=confusion_matrix(y_test_label,y_pred_label,labels=classes,normalize='true')
disp = ConfusionMatrixDisplay(confusion_matrix=confusionMatrix,display_labels=classes)
disp.plot()

tf.keras.utils.plot_model(model, to_file='pippo.pdf', show_shapes=True,rankdir='TB',show_layer_names=False)
