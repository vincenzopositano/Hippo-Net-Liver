...
data_dir ='/home/positano/Desktop/DEEP_LEARNING'
...
def frames_crop(frames,size):
    ...
    crop=ndimage.zoom(crop,1.4)
    dz=np.int32(np.ceil((np.int32(crop.shape[0])-size[0])/2.0))
    crop=crop[dz:size[0]+dz,dz:size[1]+dz]
...
def normalizeData(Xor):
    ...
    for i in range(dataSize):
        orXData=Xor[i,:,:,:] # append original data
...
def createAugmentedData1(Xor,Yor,inc):
    ...
    for i in range(dataSize):
        ...
        for j in range(inc):
            ...
            if (p ==1):
                    augImg=tf.keras.preprocessing.image.random_rotation(augImg,20,channel_axis=2,fill_mode='nearest')
            if (p ==2):
                augImg=tf.keras.preprocessing.image.random_zoom(augImg,[1.0,1.6],channel_axis=2,fill_mode='nearest')
            if (p ==3):
                    augImg=tf.keras.preprocessing.image.random_shift(augImg,0.1,0.1,channel_axis=2,fill_mode='nearest')
            if (p ==4):
                    rfloat = 1+rng.random()
                    augImg=augImg*rfloat
            if (p ==5):
...
model.add(Conv2D(filters = bSize, kernel_size = (kSize, kSize), padding='same', activation='relu',kernel_initializer='he_normal',input_shape = (img_height, img_width,seq_len)))
model.add(BatchNormalization())
model.add(MaxPooling2D(pool_size=(2, 2)))

model.add(Conv2D(filters = bSize, kernel_size = (kSize,kSize), padding='same',activation='relu',kernel_regularizer=tf.keras.regularizers.l2(l=regValue)))
model.add(BatchNormalization())
model.add(MaxPooling2D(pool_size=(2, 2)))
...
dot_img_file = '/home/positano/Desktop/DEEP_LEARNING/model_2D.png'
...
history = model.fit(x = X_train, y= y_train, epochs=epochs,batch_size = batchSize , class_weight=classW,steps_per_epoch=30,shuffle=True, validation_data=(X_val,y_val),callbacks=callbacks)
...
model.save('/home/positano/Desktop/DEEP_LEARNING/model_current_2D_88')
